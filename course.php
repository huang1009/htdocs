    ...
    <div class="container-fluid m-0 p-0">
        ...
        <div class="col-12 p-0">
            <div class="jumbotron m-0 p-0 pt-5 text-center" style="background-color:unset">
                <h1 class="display-4 animated fadeInUp">課程介紹</h1>
        ...
        <div class="row m-auto p-0 col-12 col-sm-12 col-md-9 col-lg-9 col-xl-9 pt-9">
            ...
            <?php
            $connNews = "SELECT * FROM course ORDER BY c_id ASC";
            ...
            if ($row != '') {
                do {
                    ...
                    <div class="col-12 col-sm-12 col-md-3 col-lg-3 col-xl-3 pb-5">
                        <div class="card">
                            <img src="<?php echo $row['c_img']; ?>" class="card-img-top" height="300px">
                            <div class="card-body">
                                <h5 class="card-title ex"><?php echo $row['c_name']; ?></h5>
                                <p class="card-text ellipsis"><?php echo $row['c_content']; ?></p>
                            </div>
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item">報名截止日：<?php echo $row['c_applydate']; ?></li>
                                <li class="list-group-item">上課日期：<?php echo $row['c_startdate']; ?></li>
                                <li class="list-group-item">上課地址：<?php echo $row['c_address']; ?></li>
                            </ul>
                            <div class="card-body m-auto">
                                <a href="#" class="card-link">了解詳情</a></div>

                            <?php if (@$_SESSION["UserAccounts"] != '') { ?>
                                <div class="card-body m-auto">
                                    <form action="" method="post" name="addcourse">
                                        <input name="action" type="hidden" value="addcourse">
                                        <input name="ccid" type="hidden" value="<?php echo $row['c_id']; ?>">
                                        <?php
                                                    $CheckClass = "SELECT * FROM class ORDER BY cs_id ASC";
                                                    $CheckClassResult = mysqli_query($sql_link, $CheckClass);
                                                    $CheckClassRow = mysqli_fetch_assoc($CheckClassResult);
                                                    $i = 0;
                                                    if ($CheckClassRow != '') {
                                                        do {
                                                            if ($CheckClassRow['cc_id'] ==  $row['c_id']) { ?>
                                                    <input type="submit" class="card-link" disabled=false value="已報名">
                                            <?php $i++;
                                                                }
                                                            } while ($CheckClassRow = mysqli_fetch_assoc($CheckClassResult));
                                                        }
                                                        if ($i != 1) { ?>
                                            <input type="submit" class="card-link" value="報名">
                                        <?php
                                                    }
                                                    ?>
                                    </form>
